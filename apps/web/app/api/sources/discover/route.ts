...
import { discoverAndPersistStreams } from "@/lib/source-registry";
  ...
  if (wantsStream(request)) {
    ...
    const stream = new ReadableStream({
      async start(controller) {
        ...
        try {
          const manifest = await discoverAndPersistStreams(profile, {
            onPhase: (phase, detail) => emit({ type: "phase", phase, detail }),
            signal: request.signal,
          });
          emit({ type: "result", payload: manifest });
        } catch (error) {
          if (isDiscoveryAborted(error)) {
            emit({ type: "error", message: t("sources.discoveryCancelled") });
            return;
          }
          emit({
            type: "error",
            message: error instanceof Error ? error.message : t("sources.errors.discoverFailed"),
          });
        } finally {
          controller.close();
        }
